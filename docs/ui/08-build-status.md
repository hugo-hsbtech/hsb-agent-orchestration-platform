...
| 2 · Workflow — end to end (create → configure → ready) | `473:73` | y=1280 | Workflow wizard tail + Definition (DSL) + Created + Validate ✓ + Promote + Live (y=1400) — **band label renamed 2026-05-26 (batch 6)**; see workflow end-to-end tail below |
...
The workflow stepper was extended **4 → 7 dots** to carry the new tail: Type · Identity · Model · Triggers · Steps · Budget · Review. (The relocated Triggers frame `106:333` stepper label was **corrected to "Step 4 of 7" in batch 6** — text node `108:147` — resolving the prior "Step 4 of 4" inconsistency vs. the 7-dot tail.)

| Frame | Step | Node ID | x | Stepper |
|---|---|---|---|---|
| 1 | Workflow tail — Triggers picker (Manual/Cron/Webhook/Event) — RELOCATED from Row 1 | `106:333` | 0 | **Step 4 of 7** (fixed batch 6) |
...
Steppers: chatbot frames show 1→7 of 7; chatbot Created shows all-7 teal (Step 7 of 7); workflow Triggers `106:333` now reads Step 4 of 7 (fixed batch 6); the workflow tail W2–W5 use the extended 7-dot path (Steps=5, Budget=6, Review=7, Created=all done). All text binds Paper & Signal styles by name; machine register uses JetBrains Mono.

#### Workflow end-to-end tail ✅ batch 6 2026-05-26 — Definition (DSL) + validate/promote/live

Completes the Row 2 workflow journey so it reads **create → configure → see the data (DSL) → validate ✓ → promote → live**, mirroring the chatbot Row 1 tail in the same style. The new Definition frame was inserted at x=6160 (where Created used to sit); **Created `504:80` relocated 6160 → 7700** to make room. The ready-phase tail (Validate/Promote/Live) appends after Created using the **version-lifecycle indicator** (Draft→Staging→Canary→Production pills) in place of the wizard stepper — identical to the chatbot tail. All frames 1440×1024 on the Takeover-shell pattern (top bar `Conductor · New agent` + `Exit ✕`). Built by cloning the chatbot tail frames (`538:84`/`547:84`/`547:122`/`547:160`) and rewriting content. All fills/text bind Paper & Signal variables by name (verified via `get_variable_defs` on `568:88` and `575:88` — only named tokens `state/*`, `state/*-wash`, `surface/*`, `text/*`, `border/hairline`, `brand/tide*` + the badge's `px-2,5` spacing var; no bare hex). Machine register (DSL JSON, version, ids, metrics, split %) uses JetBrains Mono. State colors used only for state/lifecycle. No 100×100 artifacts.

| Frame | Node ID | x | Lifecycle | Key content |
|---|---|---|---|---|
| **Definition (DSL)** | `568:88` | 6160 | (final wizard step — keeps 7-dot stepper, Step 7 of 7) | Heading "Definition — the agent is data"; caption "The Visual Builder graph is rendered FROM this DSL — the DSL is the source of truth."; `✓ Validates · 0 errors` chip (`state/production` on `state/production-wash`); `surface/sunken` Mono DSL well (`568:118`/`568:119`) with the full **refund-workflow DSL JSON** (`agent_type:"workflow"`, `name:"refund-workflow"`, `namespace:"acme"`, `version:"0.1"`, `model_config{claude · claude-sonnet-4-6 · 0.2}`, `trigger:{type:"manual"}`, `workflow.steps[ classify(llm_call, model_config_override→claude-haiku-4-6) · eligible(branch, "amount <= 500") · refund(tool_call, refund_api.create) · log(mcp_call, linear/create_issue) · shape(transform, mapping) ]`, `budget{max_total_tokens:200000, max_estimated_cost_usd:5.00, on_exceeded:"cancel"}`). Footer `Back` / `Create draft & open builder`. |
| **Validate ✓** | `571:88` | 9240 | Draft active | Heading "Validation passed"; green ✓ circle (`state/production-wash`); subtitle "…refund-workflow has 0 errors — all checks resolve. Ready to promote." ([AD-13] collect-all); `surface/sunken` checklist (`571:117`+) of 5 workflow-specific passed checks (each green ✓): tools registered & in namespace (`refund_api.create` resolves in acme) · MCP capability in manifest (`linear · create_issue` published) · `max_concurrency` present where required (for_each/parallel OK) · no cycles (DAG acyclic, max depth 4 ≤ 10) · budget reachable (price known for all models · est. ≤ $5.00 cap). Footer `Back` / `Promote →`. |
| **Promote (lane)** | `573:88` | 10780 | Canary active | **Promotion Lane** (mirrors chatbot `547:122` / Version Promotion screen `48:40`): heading "Promote refund-workflow `v1.0`"; 4 stage cards Draft `v1.0`/Staging `v1.0`/**Canary `v1.0`** (amber `state/canary-wash`+stroke, active)/Production `v0.9`; canary traffic-split bar v1 10% (`state/canary`) / v0 90% (`state/production`); promotion-gate chips: ✓ eval gate 4.6/5 ≥ 4.0 · ✓ error-rate gate 0.2% < 2% · ↻ auto-rollback armed. Footer `Back` / `Promote to production`. |
| **Live / ready** | `575:88` | 12320 | Production active (complete) — READY terminal | Heading "Live"; green icon; **Version State Badge Production instance** (`575:116`, from `20:11`); `refund-workflow v1.0` (Mono); "Ready to run · triggered manually / on schedule"; 3-cell stats card (`575:119`, widened to 560 + cloned third cell `579:92`): `48` RUNS TODAY · `12.4 s` P50 DURATION · `$0.21` AVG COST (Mono); links `View Runs` (`51:56`) · `open Visual Builder` (`35:34`) in `brand/tide-text`. Footer `Back to Catalogue` / `Done`. |

**Batch 6 also:** band label `473:73` renamed `2 · Workflow path` → **`2 · Workflow — end to end (create → configure → ready)`**; Triggers stepper text `108:147` fixed `Step 4 of 4` → **`Step 4 of 7`**; Created `504:80` relocated x=6160 → **x=7700**.
...
**Agent journey end-to-end COMPLETE ✅ — all 4 bands, 29 frames total (Row 1: 12 frames — chatbot end-to-end · Row 2: 9 frames — workflow end-to-end · Row 3: 4 frames · Row 4: 5 frames). Both primary paths (chatbot Row 1, workflow Row 2) now read create → configure → see the data (DSL) → validate ✓ → promote → live, with matching version-lifecycle tails.**
