...
- [ ] **Lists pattern** (in progress): filter bar + Blocks⇄Datatable view toggle + dual pagination (blocks = infinite scroll, datatable = numbered) sharing **one paginated datasource signature**, applied across the list screens. **Task L0 DONE 2026-05-25** — reusable constructs built on the Components page (`FilterBar` `280:37`, `ViewToggle` `279:67`, `BlockGrid sentinel` `285:70`, `DataTable + Pager` `286:50`) + the new `Lists pattern` page (`278:2`) for later applied/state frames.
  - **Task L1 DONE 2026-05-25 — Lists · Agent Catalogue:**
    ...
    - Remaining: apply same pattern to other list screens (Runs `51:56`, Tool & MCP Catalogue `56:56`, KB Management `68:56`, Marketplace `71:56`).
